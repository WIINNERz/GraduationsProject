...
import { View, Text, StyleSheet, TextInput, Button, ActivityIndicator, TouchableOpacity,Alert } from 'react-native';
...
export default function PetDetail() {
    ...
    const handleSave = async () => {
        try {
            ...
            await setDoc(doc(db, 'Pets', newId), { ...pet, id: newId });
            navigation.goBack();
        } catch (err) {
            setError(err.message);
        }
    ...
    if (loading) {
        return <ActivityIndicator size="large" color="#E16539"/>;
    }
